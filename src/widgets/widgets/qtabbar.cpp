...
QRect QTabBarPrivate::tabBarScrollRect()
{
    Q_Q(QTabBar);
    QStyleOption opt;
    opt.init(q);
...
void QTabBar::paintEvent(QPaintEvent *)
{
    ...
    const QRect scrollRect = d->tabBarScrollRect();
    ...
    for (int i = 0; i < d->tabList.count(); ++i) {
        ...
        if (!vertical) {
            ...
            if ((!rtl && tabScrolledOutLeft) || (rtl && tabScrolledOutRight)) {
                cutLeft = i;
                cutTabLeft = tab;
            } else if ((!rtl && tabScrolledOutRight) || (rtl && tabScrolledOutLeft)) {
                cutRight = i;
                cutTabRight = tab;
            }
        } else {
            bool tabScrolledOutTop = tab.rect.top() < 0;
            bool tabScrolledOutBottom = tab.rect.bottom() > height();
            if (tabScrolledOutTop) {
                cutLeft = i;
                cutTabLeft = tab;
            } else if (tabScrolledOutBottom){
                cutRight = i;
                cutTabRight = tab;
            }
        }
