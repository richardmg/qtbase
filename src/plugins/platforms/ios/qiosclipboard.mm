...
QVariant QIOSMimeData::retrieveData(const QString &mimeType, QVariant::Type) const
{
    ...
    foreach (QMacInternalPasteboardMime *convertor,
             QMacInternalPasteboardMime::all(QMacInternalPasteboardMime::MIME_ALL)) {
        if (!convertor->canConvert(mimeType, convertor->flavorFor(mimeType)))
            continue;
        ...
        for (NSUInteger i = 0; i < [pasteboardTypes count]; ++i) {
            ...
            if (!convertor->canConvert(mimeType, availableUti))
                continue;
            ...
            return convertor->convertToMime(mimeType, dataList, availableUti);
        }
    }
...
void QIOSClipboard::setMimeData(QMimeData *mimeData, QClipboard::Mode mode)
{
    ...
    foreach (const QString &mimeType, mimeData->formats()) {
        foreach (QMacInternalPasteboardMime *convertor,
                 QMacInternalPasteboardMime::all(QMacInternalPasteboardMime::MIME_ALL)) {
            QString uti = convertor->flavorFor(mimeType);
            if (uti.isEmpty() || !convertor->canConvert(mimeType, uti))
                continue;

            QByteArray byteArray = convertor->convertFromMime(mimeType, mimeData->data(mimeType), uti).first();
