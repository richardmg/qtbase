...
- (id)initWithVisibleMenuItems:(const QIOSMenuItemList &)visibleMenuItems targetRect:(const QRect &)targetRect
{
    if (self = [super init]) {
        ...
        [menuController setTargetRect:toCGRect(targetRect) inView:view];

        if (QWindow *window = qGuiApp->focusWindow()) {
            // The first responder will get the menu action callbacks, so we need to set it:
            [reinterpret_cast<UIView *>(window->winId()) becomeFirstResponder];
        }
    }
...
- (id)initWithVisibleMenuItems:(const QIOSMenuItemList &)visibleMenuItems title:(const QString &)title targetRect:(const QRect &)targetRect
{
    ...
    if (self) {
        ...
        UIView *view = [UIApplication sharedApplication].keyWindow.rootViewController.view;
        [self showFromRect:toCGRect(targetRect) inView:view animated:YES];
    }
...
void QIOSMenu::updateVisibilityUsingUIMenuController()
{
    if (m_effectiveVisible) {
        ...
        m_menuController = [[QUIMenuController alloc] initWithVisibleMenuItems:visibleMenuItems() targetRect:m_targetRect];
...
void QIOSMenu::updateVisibilityUsingUIActionSheet()
{
    if (m_effectiveVisible) {
        ...
        m_actionSheet = [[QUIActionSheet alloc] initWithVisibleMenuItems:visibleMenuItems() title:m_text targetRect:m_targetRect];
    } else {
        Q_ASSERT(m_actionSheet);
