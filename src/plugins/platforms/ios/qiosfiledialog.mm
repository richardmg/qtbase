...
- (id)initWithQIOSFileDialog:(QIOSFileDialog *)fileDialog parent:(QWindow *)parent
{
    self = [super init];
    if (self) {
...
bool QIOSFileDialog::show(Qt::WindowFlags windowFlags, Qt::WindowModality windowModality, QWindow *parent)
{
    ...
    QString systemPictureLocation = QStandardPaths::standardLocations(QStandardPaths::PicturesLocation).last();

    if (options()->initialDirectory() == systemPictureLocation) {
        m_viewController = [[QIOSImagePickerController alloc] initWithQIOSFileDialog:this parent:parent];
        return true;
    }
