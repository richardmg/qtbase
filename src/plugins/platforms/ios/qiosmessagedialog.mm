...
inline UIAlertAction *QIOSMessageDialog::createAction(StandardButton button)
{
    ...
    const QString label = removeMnemonics(QGuiApplicationPrivate::platformTheme()->standardButtonText(labelButton));
...
bool QIOSMessageDialog::show(Qt::WindowFlags windowFlags, Qt::WindowModality windowModality, QWindow *parent)
{
    ...
    if (m_alertController // Ensure that we're not showing the dialog already
            || !options() // Some message dialogs don't use options (QErrorMessage)
            || windowModality == Qt::NonModal // iOS only have use modal dialogs
            || QSysInfo::MacintoshVersion < QSysInfo::MV_IOS_8_0) // API limitation
        return false;

    // We don't distinguish between window modal and application modal dialogs on iOS.
    // UIAlertControllerStyleActionSheet is an action menu that (on iPad) will require
    // a rectangle for the dialog to point to. So we use UIAlertControllerStyleAlert for all.
    ...
    StandardButtons buttons = options()->standardButtons();
    if (!buttons) {
        // We need at least one button to let the user close the dialog
